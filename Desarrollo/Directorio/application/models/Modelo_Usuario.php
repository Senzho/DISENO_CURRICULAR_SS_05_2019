...
class Modelo_Usuario extends CI_Model implements I_Usuario {
    ...
    public function existe($nombre_usuario, $contraseña) {
        ...
        $consulta = $this->db->get_where('usuario', array('usuario_nick' => $nombre_usuario, 'usuario_contraseña' => $contraseña));
        $existe = ($consulta->num_rows() === 1) ? TRUE : FALSE;
        return $existe;
    }
    public function obtener_por_credenciales($nombre_usuario, $contraseña) {
        $this->load->database('diseñoCurricular');
        $consulta_diseño = $this->db->get_where('usuario', array('usuario_nick' => $nombre_usuario, 'usuario_contraseña' => $contraseña));
        $usuario = new Usuario();
        ...
        $siu_db = $this->load->database('siu', TRUE);
        $consulta_siu = $siu_db->get_where('personal', array('numeroPersonal' => $consulta_diseño->row()->usuario_siu_id));
